...
class Analyze_LAMMPS:
    ...
    def ImCorrelation_molecule_symmetric(self, skip_frac=0.2, beta=1, hbar=1, use_kg_mass_units=True, dim=3, output_name='G.csv',pbc=None):
        '''
        GENERATES A CSV FILE OF IMAGINARY TIME CORRELATION FUNCTION FOR MOLECULES
        '''
        ...
        for ibead in tqdm(range(beads)):
            ...
            for k in range(beads):
                ...
                if pbc is not None:
                    ...
                    R_k_ibead_k1_ibead[R_k_ibead_k1_ibead<-pbc/2] += pbc
                addition+= (R_k_k1*R_k_ibead_k1_ibead).sum()/(steps-drop)/particles
            G[ibead] = int(0==ibead)*( beads*dim/(masses*beta) ).mean() - addition * beads/(hbar*beta)**2
